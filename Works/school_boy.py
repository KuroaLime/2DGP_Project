...
FRAMES_PER_ACTION_ATK = 8
FRAMES_PER_ACTION_STUN = 3
FRAMES_PER_ACTION_DEAD = 26
animation_names = ['Idle', 'Walk','Axe_kick','Stun_one','Stun_two','Stun_three','Dead']
class School_boy:
    images = None
    Get_hit_sound = None
    def load_images(self):
        if School_boy.images == None:
            School_boy.images = {}
            for name in animation_names:
                if name == 'Axe_kick':
                    School_boy.images[name] = [load_image(
                        "./Resource/classification/enermy/school_boy/Occupied/" + name + " (%d)" % i + ".png") for i in
                        range(1, 9)]
    ...
    def __init__(self):
        ...
        if School_boy.Get_hit_sound is None:
            School_boy.Get_hit_sound = load_wav('sound/Get_hit_enermy/gethit_knockdown_02.wav')
            School_boy.Get_hit_sound.set_volume(self.effect_Volume)
        ...
        if self.stun_state == True:
            if server.Player.attack_turn == 0:
                if math.cos(self.dir) < 0:
                    School_boy.images['Stun_one'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    School_boy.images['Stun_one'][int(self.frame)].draw(sx, sy, 190, 200)
            elif server.Player.attack_turn == 1:
                if math.cos(self.dir) < 0:
                    School_boy.images['Stun_two'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    School_boy.images['Stun_two'][int(self.frame)].draw(sx, sy, 190, 200)
            elif server.Player.attack_turn == 2:
                if math.cos(self.dir) < 0:
                    School_boy.images['Stun_three'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    School_boy.images['Stun_three'][int(self.frame)].draw(sx, sy, 190, 200)
        elif self.dead_state == True:
            if math.cos(self.dir) < 0:
                School_boy.images['Dead'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                School_boy.images['Dead'][int(self.frame)].draw(sx, sy, 190, 200)
        elif self.atk_state == True:
            if math.cos(self.dir) < 0:
                School_boy.images['Axe_kick'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                School_boy.images['Axe_kick'][int(self.frame)].draw(sx, sy, 190, 200)
        else:
            if math.cos(self.dir) < 0:
                if self.speed == 0:
                    School_boy.images['Idle'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    School_boy.images['Walk'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                if self.speed == 0:
                    School_boy.images['Idle'][int(self.frame)].draw(sx, sy, 190, 200)
                else:
                    School_boy.images['Walk'][int(self.frame)].draw(sx, sy, 190, 200)
