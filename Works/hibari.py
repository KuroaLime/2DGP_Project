...
class Hibari:
    ...
    def load_images(self):
        if Hibari.images == None:
            Hibari.images = {}
            for name in animation_names:
                Hibari.images[name] = [load_image("./Resource/classification/Boss/Hibari/Occupied/"+ name + " (%d)" % i + ".png") for i in range(1, 9)]

    def prepare_patrol_points(self):
        positions = [(700, 650), (1118, 750), (1050, 530), (575, 220), (235, 33), (575,220), (1050, 530), (1118,750)]
        self.patrol_points = []
        for p in positions:
    ...
    def draw(self):
        ...
        if math.cos(self.dir) < 0:
            if self.speed == 0:
                Hibari.images['Idle'][int(self.frame)].composite_draw(0, 'h', self.x, self.y, 190, 200)
            else:
                Hibari.images['Walk'][int(self.frame)].composite_draw(0, 'h', self.x, self.y, 190, 200)
        else:
            if self.speed == 0:
                Hibari.images['Idle'][int(self.frame)].draw(self.x, self.y, 190, 200)
            else:
                Hibari.images['Walk'][int(self.frame)].draw(self.x, self.y, 190, 200)
