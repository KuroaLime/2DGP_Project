...
animation_names = ['Idle', 'Walk','Smack','Stun_one','Stun_two','Stun_three','Dead']
class Police_man:
    images = None
    Get_hit_sound = None
    def load_images(self):
        if Police_man.images == None:
            ...
            for name in animation_names:
                if name == 'Smack':
                    Police_man.images[name] = [load_image(
                        "./Resource/classification/enermy/police_man/Occupied/" + name + " (%d)" % i + ".png") for i in
                        range(1, 13)]
    ...
    def __init__(self):
        ...
        if Police_man.Get_hit_sound is None:
            Police_man.Get_hit_sound = load_wav('sound/Get_hit_enermy/gethit_knockdown_02.wav')
            Police_man.Get_hit_sound.set_volume(self.effect_Volume)
        ...
        if self.stun_state == True:
            if server.Player.attack_turn == 0:
                if math.cos(self.dir) < 0:
                    Police_man.images['Stun_one'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    Police_man.images['Stun_one'][int(self.frame)].draw(sx, sy, 190, 200)
            elif server.Player.attack_turn == 1:
                if math.cos(self.dir) < 0:
                    Police_man.images['Stun_two'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    Police_man.images['Stun_two'][int(self.frame)].draw(sx, sy, 190, 200)
            elif server.Player.attack_turn == 2:
                if math.cos(self.dir) < 0:
                    Police_man.images['Stun_three'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    Police_man.images['Stun_three'][int(self.frame)].draw(sx, sy, 190, 200)
        elif self.dead_state == True:
            if math.cos(self.dir) < 0:
                Police_man.images['Dead'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                Police_man.images['Dead'][int(self.frame)].draw(sx, sy, 190, 200)
        elif self.atk_state == True:
            if math.cos(self.dir) < 0:
                Police_man.images['Smack'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                Police_man.images['Smack'][int(self.frame)].draw(sx, sy, 190, 200)
        else:
            if math.cos(self.dir) < 0:
                if self.speed == 0:
