...
FRAMES_PER_ACTION_IDLE = 10
FRAMES_PER_ACTION_RUN = 10
FRAMES_PER_ACTION_ATK = 11
FRAMES_PER_ACTION_STUN = 3
FRAMES_PER_ACTION_DEAD = 13
animation_names = ['Idle', 'Walk','Elbow','Stun_one','Stun_two','Stun_three','Dead']

class Misuzu:
    images=None
    Get_hit_sound=None
    def load_images(self):
        if Misuzu.images == None:
            Misuzu.images = {}
            for name in animation_names:
                if name == 'Elbow':
                    Misuzu.images[name] = [load_image(
                        "./Resource/classification/Boss/Misuzu/Occupied/" + name + " (%d)" % i + ".png") for i in
                        range(1, 12)]
                    ...
                    Misuzu.images[name] = [load_image("./Resource/classification/Boss/Misuzu/Occupied/"+ name + " (%d)" % i + ".png") for i in range(1, 11)]
    ...
    def __init__(self):
        ...
        if Misuzu.Get_hit_sound is None:
            Misuzu.Get_hit_sound = load_wav('sound/Get_hit_enermy/gethit_knockdown_02.wav')
        ...
        if self.stun_state == True:
            if server.Player.attack_turn == 0:
                if math.cos(self.dir) < 0:
                    Misuzu.images['Stun_one'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    Misuzu.images['Stun_one'][int(self.frame)].draw(sx, sy, 190, 200)
            elif server.Player.attack_turn == 1:
                if math.cos(self.dir) < 0:
                    Misuzu.images['Stun_two'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    Misuzu.images['Stun_two'][int(self.frame)].draw(sx, sy, 190, 200)
            elif server.Player.attack_turn == 2:
                if math.cos(self.dir) < 0:
                    Misuzu.images['Stun_three'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    Misuzu.images['Stun_three'][int(self.frame)].draw(sx, sy, 190, 200)
        elif self.dead_state == True:
            if math.cos(self.dir) < 0:
                Misuzu.images['Dead'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                Misuzu.images['Dead'][int(self.frame)].draw(sx, sy, 190, 200)
        elif self.atk_state == True:
            if math.cos(self.dir) < 0:
                Misuzu.images['Elbow'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                Misuzu.images['Elbow'][int(self.frame)].draw(sx, sy, 190, 200)
        else:
            if math.cos(self.dir) < 0:
                if self.speed == 0:
