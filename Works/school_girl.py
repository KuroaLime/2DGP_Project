...
FRAMES_PER_ACTION_ATK = 7
FRAMES_PER_ACTION_STUN = 3
FRAMES_PER_ACTION_DEAD = 24
animation_names = ['Idle', 'Walk','Jab','Stun_one','Stun_two','Stun_three','Dead']
class School_girl:
    images=None
    Get_hit_sound=None
    def load_images(self):
        if School_girl.images == None:
            ...
            for name in animation_names:
                if name == 'Jab':
                    School_girl.images[name] = [load_image(
                        "./Resource/classification/enermy/school_girl/Occupied/" + name + " (%d)" % i + ".png") for i in
                                                range(1, 8)]
    ...
    def __init__(self):
        ...
        if School_girl.Get_hit_sound is None:
            School_girl.Get_hit_sound = load_wav('sound/Get_hit_enermy/gethit_knockdown_02.wav')
            School_girl.Get_hit_sound.set_volume(self.effect_Volume)
    ...
    def atk_player(self):
        ...
        if server.Player.attacking == True:
            ...
            if self.atk_timer <= 0:
                ...
                self.atk_state = False
                School_girl.Get_hit_sound.play()
                return BehaviorTree.SUCCESS
            else:
                return BehaviorTree.RUNNING
        ...
        if self.stun_state == True:
            if server.Player.attack_turn == 0:
                if math.cos(self.dir) < 0:
                    School_girl.images['Stun_one'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    School_girl.images['Stun_one'][int(self.frame)].draw(sx, sy, 190, 200)
            elif server.Player.attack_turn == 1:
                if math.cos(self.dir) < 0:
                    School_girl.images['Stun_two'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    School_girl.images['Stun_two'][int(self.frame)].draw(sx, sy, 190, 200)
            elif server.Player.attack_turn == 2:
                if math.cos(self.dir) < 0:
                    School_girl.images['Stun_three'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    School_girl.images['Stun_three'][int(self.frame)].draw(sx, sy, 190, 200)
        elif self.dead_state == True:
            if math.cos(self.dir) < 0:
                School_girl.images['Dead'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                School_girl.images['Dead'][int(self.frame)].draw(sx, sy, 190, 200)
        elif self.atk_state == True:
            if math.cos(self.dir) < 0:
                School_girl.images['Jab'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                School_girl.images['Jab'][int(self.frame)].draw(sx, sy, 190, 200)
        else:
            if math.cos(self.dir) < 0:
                if self.speed == 0:
                    School_girl.images['Idle'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
                else:
                    School_girl.images['Walk'][int(self.frame)].composite_draw(0, 'h', sx, sy, 190, 200)
            else:
                if self.speed == 0:
                    School_girl.images['Idle'][int(self.frame)].draw(sx, sy, 190, 200)
                else:
                    School_girl.images['Walk'][int(self.frame)].draw(sx, sy, 190, 200)
